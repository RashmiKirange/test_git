...
static double speed1=0;
static double speed2=0;
...
static void IRAM_ATTR N1()
{
  ...
  if (currentMicros1 - lastDebounceTime1 > debounceDelay)
  {
    ...
    distance1=distance1+ (2*3.14*0.27)/(4000);
...
static void IRAM_ATTR N2()
{
  ...
  if (currentMicros2 - lastDebounceTime2 > debounceDelay)
  {
    ...
    distance2=distance2+ (2*3.14*0.27)/(4000);
  }
}
...
void setup() {
  ...
  writeFile(SD, FileName.c_str(), "Micros,Voltage,Temperature,SOC,PC,DCL,Failsafe,DTC1,DTC2,M_RPM,M_Temp,MC_Temp,M_Torque,M_Curr,M_Vol,RPM_L,Speed_L,Distance_L,RPM_R,Speed_R,Distance_R\n");  //print this string first into the file
...
void loop() {
  ...
  if(flag_rpm1) {
    RPMN_R = (60.533*1000000)/(RPMN1period*6);
    //RPMN_R=(60/(RPMN1period*slot))*1000000;
    //speed = 
    flag_rpm1=0;
  }
  if(flag_rpm2) {
    RPMN_L = (60.533*1000000)/(RPMN2period*6);
    //RPMN_L=((60*1000000)/(RPMN2period*slot));
    //speed = (2 * 3.14* 0.27 * RPMN_R)/60 
    flag_rpm2=0;
  }
  if((micros()-lastDebounceTime1)>1000000) RPMN_R=0.0;
  if((micros()-lastDebounceTime2)>1000000) RPMN_L=0.0;
  //Serial.println("Failing");} 
  speed1=(2*3.14*RPMN_R*0.27*3.6)/60;
  speed2=(2*3.14*RPMN_L*0.27*3.6)/60;
  ...
  //float current = (voltage-2.5)/0.066;
   ...
   String Data = String(micros()) + "," + String(voltage) + "," + String(temperatureHigh) + "," + String(packSoC) + "," + String(PC) + "," + String(DCL) + "," + String(failsafe) + "," + String(DTC1) + "," + String(DTC2) +"," + String(rpm) + "," + String(motor_temp) + "," + String(controller_temp) + "," + String(torque) + "," + String(motor_current) + "," + String(motor_voltage)+","+String(RPMN_L)+","+String(speed2)+","+String(distance2)+","+String(RPMN_R)+","+String(speed1)+","+String(distance1)+"\n";
